...
- [Задание 24. Template Method](#задание-24-template-method)
- [Задание 25. Visitor](#задание-25-visitor)
...
В данном разделе реализован паттерн **Observer**, который позволяет объекту (наблюдателю) автоматически получать уведомления о изменениях в другом объекте (наблюдаемом). Этот паттерн полезен, когда необходимо обновлять состояние зависимых объектов при изменении состояния наблюдаемого объекта. Например, при добавлении нового события в систему уведомления могут отправляться всем подписанным на это событие пользователям или компонентам системы.

### Причины выбора Observer для проекта

1. **Асинхронная реакция на изменения**: Паттерн Observer позволяет нескольким объектам быть уведомленными о событии, не блокируя основной поток работы. Это особенно важно в многозадачных приложениях, где уведомления об изменениях могут быть обработаны в разных частях системы.

2. **Управление подписками и уведомлениями**: Паттерн позволяет добавлять или удалять наблюдателей в любое время, без изменения кода самого наблюдаемого объекта. Это дает гибкость в реализации бизнес-логики и динамическое управление событиями.

3. **Снижение связанности компонентов**: Система становится менее зависимой от конкретных реализаций компонентов. Наблюдатель реагирует только на изменения, не зная деталей реализации источника событий, что упрощает тестирование и расширяемость системы.

### Признаки реализации Observer в проекте

- **Интерфейс Observer**: Класс, который реализует интерфейс `Observer`, предоставляет метод `update()`, который будет вызываться при изменении состояния наблюдаемого объекта.

- **Интерфейс Observed**: Наблюдаемый объект должен реализовывать интерфейс `Observed`, который включает методы для добавления, удаления и уведомления наблюдателей.

- **Использование события в сервисах**: В примере с сервисом `EventService` при создании или обновлении события система уведомляет всех зарегистрированных наблюдателей о произошедшем изменении, что позволяет, например, записывать логи, отправлять уведомления или обновлять состояние интерфейса.

- **Конфигурация с Spring**: Наблюдатели добавляются через конфигурацию `ObserverConfig`, где подписка на события происходит автоматически через внедрение зависимостей и использование Spring Beans.
...
В этом разделе описан класс [`Event`](./src/main/java/ru/patterns/models/Event.java), который реализует паттерн проектирования State. Паттерн State позволяет объекту изменять свое поведение в зависимости от его состояния. Состояние объекта инкапсулируется в отдельные классы, и каждый класс состояния управляет поведением объекта в этом состоянии.

### Причины выбора State для класса `Event`

1. **Динамическое изменение поведения**: Класс `Event` может находиться в одном из нескольких состояний (например, "создано", "обновлено", "завершено", "отменено"). В зависимости от текущего состояния, поведение события изменяется. Паттерн State позволяет избежать условных операторов, которые бы меняли поведение объекта в зависимости от его состояния.

2. **Инкапсуляция состояний**: Каждый из возможных состояний события (например, CreatedState, UpdatedState, CompletedState, CanceledState) вынесен в отдельный класс. Это упрощает расширение системы и поддержку новых состояний, а также улучшает читаемость и поддерживаемость кода.

### Признаки реализации State в классе `Event`

- **Интерфейс состояния**: Класс `Event` использует интерфейс `EventState`, который определяет метод `handle()`. Этот метод вызывается для изменения состояния события в зависимости от текущего состояния. Все классы состояний (`CreatedState`, `UpdatedState`, `CompletedState`, `CanceledState`) реализуют этот интерфейс и определяют свою логику обработки событий.

- **Переключение состояний**: Каждый класс состояния изменяет текущее состояние объекта `Event`. Например, из состояния "создано" объект может перейти в состояние "обновлено", затем в "завершено", и, наконец, в "отменено". Переключение между состояниями происходит через метод `setState()`.

- **Обработка состояний**: В методе `handle()` каждого состояния выполняются специфичные для этого состояния действия (например, вывод сообщений в консоль). Когда состояние обработано, объект события переходит в следующее состояние.
...
В этом разделе описан пример реализации паттерна **Strategy** для обработки различных типов заметок. Паттерн **Strategy** позволяет определять семейство алгоритмов и инкапсулировать их в отдельных классах, обеспечивая возможность выбирать алгоритм на этапе выполнения.

В данной реализации существует несколько типов заметок, и для каждого типа используется своя стратегия обработки:

- **Текстовые заметки** обрабатываются с помощью стратегии `TextNoteProcessingStrategy`.
- **Изображения** обрабатываются с помощью стратегии `ImageNoteProcessingStrategy`.
- **Списки задач (checklist)** обрабатываются с помощью стратегии `ChecklistNoteProcessingStrategy`.

Каждая стратегия реализует интерфейс `NoteProcessingStrategy` и предоставляет метод `process()`, который выполняет обработку данных в зависимости от типа заметки.

### Причины выбора паттерна Strategy

1. **Гибкость и расширяемость**: Паттерн позволяет добавлять новые типы заметок и соответствующие стратегии обработки, не изменяя существующий код. Это делает систему более гибкой и легко расширяемой.

2. **Изоляция логики**: Каждая стратегия инкапсулирует логику обработки конкретного типа заметки. Это помогает разделить ответственность и упростить поддержку и модификацию каждой части системы.

3. **Управление алгоритмами на лету**: Паттерн позволяет выбирать стратегию обработки в зависимости от типа заметки во время выполнения, что позволяет динамически адаптировать поведение системы.
...
В этом разделе представлен пример реализации паттерна проектирования **Template Method**. Этот паттерн определяет общий алгоритм выполнения задачи в виде абстрактного метода (шаблона), оставляя реализацию конкретных шагов подклассам. Это позволяет вынести общую логику в базовый класс и делегировать детали поведения подклассам, сохраняя архитектуру гибкой и расширяемой.

### Причины выбора Template Method для обработки тем

1. **Повторное использование кода**: Паттерн Template Method позволяет вынести общие этапы обработки темы, такие как проверка, сохранение в базу данных, в базовый класс `ThemeProcessor`. Это упрощает код и уменьшает дублирование логики.

2. **Гибкость при добавлении новых обработчиков**: Благодаря абстрактному методу `applyCustomLogic(Theme theme)`, подклассы могут определять собственное поведение для обработки темы, не изменяя базовый алгоритм.

3. **Централизованный контроль за процессом**: Базовый класс гарантирует, что каждый шаг (валидация, применение логики, сохранение) будет выполнен в правильной последовательности.

### Реализация паттерна Template Method

- **Базовый класс `ThemeProcessor`**:
    - Определяет общий алгоритм обработки темы в методе `processTheme()`.
    - Содержит шаги, которые одинаковы для всех подклассов, такие как `validateTheme` и `saveTheme`.
    - Содержит абстрактный метод `applyCustomLogic`, который реализуется в подклассах.

- **Подклассы `LightThemeProcessor` и `DarkThemeProcessor`**:
    - Реализуют метод `applyCustomLogic`, определяя индивидуальную логику для светлой и тёмной темы.
...
В этом разделе описан паттерн проектирования **Visitor**, который используется для добавления новых операций к объектам без изменения их классов. Паттерн Visitor позволяет определить новые операции, не изменяя классы объектов, к которым эти операции применяются. В данном примере используется паттерн Visitor для обработки различных аспектов темы, таких как создание отчетов и генерация изображений с использованием цветов темы.

### Причины выбора Visitor для работы с темами

1. **Добавление операций без изменения классов**: Паттерн Visitor идеально подходит для ситуации, когда нужно добавить новые операции (например, создание отчетов или генерация изображений), не меняя существующие классы объектов, таких как класс `Theme`.

2. **Гибкость в добавлении новых операций**: Паттерн позволяет легко расширять функциональность системы. Например, можно добавить новый посетитель, чтобы выполнить другие операции с темами (например, анализ или изменение цветов) без изменения существующего кода.

3. **Централизация логики обработки**: Все операции, связанные с обработкой темы, централизуются в классе-посетителе. Это позволяет изолировать логику обработки от самой структуры данных, улучшая поддержку и расширяемость.

### Признаки реализации Visitor в приложении

- **Интерфейс `ThemeVisitor`**: Интерфейс, который определяет метод `visit(Theme theme)`, позволяющий посетителю обрабатывать объект `Theme`.

- **Интерфейс `ThemeElement`**: Интерфейс, который предоставляет метод `accept(ThemeVisitor visitor)` для приема посетителя.

- **Классы-посетители (`ReportVisitor`, `GenerateImageVisitor`)**: Классы, которые реализуют интерфейс `ThemeVisitor` и содержат конкретную логику для обработки объектов `Theme`. Например, `ReportVisitor` создает отчет о теме, а `GenerateImageVisitor` генерирует изображение с использованием цветов из темы.

- **Методы `accept()` в классе `Theme`**: Класс `Theme` реализует метод `accept(ThemeVisitor visitor)`, который позволяет посетителям принимать объекты темы и выполнять соответствующие операции.
